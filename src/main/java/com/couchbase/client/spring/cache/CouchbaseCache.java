...
public class CouchbaseCache implements EnableableCache {
    ...
    private final com.github.benmanes.caffeine.cache.Cache<Object, SerializableDocument> localCache;
    ...
    /**
     * Construct the cache and pass in the {@link Bucket} instance. This constructor will create a local Caffeine
     * cache instance with maximum size of zero (immediate eviction, equivalent to no local cache).
     *
    ...
    /**
     * Construct the cache and pass in the {@link Bucket} instance. This constructor will create a local Caffeine
     * cache instance with given maximum size; the local cache is read on get and written to on put.
     *
     * @param name   the name of the cache reference.
     * @param client the Bucket instance.
    ...
    public CouchbaseCache(final String name, final Bucket client, int ttl, int initLocalCapacity, int maxLocalCapacity) {
        ...
        this.localCache = Caffeine.newBuilder()
                .initialCapacity(initLocalCapacity)
                .maximumSize(maxLocalCapacity)
                .expireAfterWrite(TimeUnit.SECONDS.toMillis(ttl)/2, TimeUnit.MILLISECONDS)
    ...
    public String getRemoteCacheStatsStr() { return "{ 'basicStats' : " + this.client.bucketManager().info().raw().get("basicStats").toString();}
    /**
    ...
    public final ValueWrapper get(final Object key) {
        ...
        SerializableDocument doc = localCache.get(documentId, (o) -> client.get(documentId, SerializableDocument.class));
    ...
    public <T> T get(final Object key, final Callable<T> valueLoader) {
        ...
        if (doc == null && valueLoader != null) {
            synchronized (client) {
                doc = localCache.get(documentId, (o) -> client.get(documentId, SerializableDocument.class));
                if (doc == null) {
                    try {
    ...
    static class CustomRemovalListener implements RemovalListener<Object, Object> {
        private static final Logger LOGGER = LoggerFactory.getLogger(CustomRemovalListener.class);

        @Override
        public void onRemoval(Object key, Object value, RemovalCause cause) {
            LOGGER.debug("onRemoval(): removal listerner called with key={}, cause={}, evicted [{}]",
