...
public class CouchbaseCache implements EnableableCache {
     ...
     */
    public CouchbaseCache(final String name, final Bucket client, final SafeObjectMapper objectMapper) {
        this.name = name;
     ...
     */
    public CouchbaseCache(final String name, final Bucket client, int ttl, final SafeObjectMapper objectMapper) {
        this.name = name;
        this.nameLength = name.length();
        this.client = client;
        this.ttl = ttl;
    ...
    public final ValueWrapper get(final Object key) {
        ...
        String[] content = doc.content().toString().split(CLASS_SEPARATOR, 2);
        try {
            return new SimpleValueWrapper(objectMapper.readValue(content[1], Class.forName(content[0])));
    ...
    public final <T> T get(final Object key, final Class<T> clazz) {
        ...
        if (doc == null) {
            ...
            return(T) objectMapper.readValue(content[1], clazz);
        }
    ...
    public <T> T get(final Object key, final Callable<T> valueLoader) {
        ...
        if (doc != null) {
            String[] content = doc.content().toString().split(CLASS_SEPARATOR, 2);
            try {
                Class<T> clazz = (Class<T>) Class.forName(content[0]);
                return (T) objectMapper.readValue(content[1], clazz);
            } catch (ClassNotFoundException e) {
                throw new RuntimeException(e);
            }
    ...
    public final void put(final Object key, final Object value) {
        if (value != null) {
            ...
            if (enabled) {
                SerializableDocument doc = SerializableDocument.create(
                        documentId,
                        ttl,
                        value.getClass().getName() + CLASS_SEPARATOR + objectMapper.writeValueAsString(value));
                client.upsert(doc);
            }
        } else {
            evict(key);
